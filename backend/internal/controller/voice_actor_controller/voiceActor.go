...
func (ac *VoiceActorController) Get(id string) (models.VoiceActor, error) {
	var voiceActor models.VoiceActor

	if err := db.DB.
		Preload("Roles").
		First(&voiceActor, id).Error; err != nil {
		return models.VoiceActor{}, errors.New("failed to retrieve voice actor")
	}
...
func (ac *VoiceActorController) Update(c *gin.Context, id string) (models.VoiceActor, error) {
	...
	if err := db.DB.First(&actorToUpdate, id).Error; err != nil {
		return models.VoiceActor{}, errors.New("failed to find voice actor")
	}
	...
	if db.DB.Save(&actorToUpdate).Error != nil {
		return models.VoiceActor{}, errors.New("failed to update voice actor")
	}
