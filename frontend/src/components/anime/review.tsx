...
import { Anime, Review, User, UserAnime } from "@/types/models";
...
const ReviewWrapper: FC<{ r: Review, my: boolean }> = ({ r, my }) => {
    return (
        <div>
            {my && (
                <div className="label">
                    ...
                    <div className="flex flex-row w-full">
                        <div className="avatar placeholder">
                            <div className="ring-gray-500 ring-offset-base-100 md:w-20 md:h-20 w-14 h-14 rounded-full ring ring-offset-2">
                                {r.userAnime.user.picUrl ? (
                                    <Image alt={`${r.userAnime.user.username}'s avatar`} src={getImageUrl(r.userAnime.user.picUrl)} width={150} height={150} />
                                ) : <span className="text-3xl">{r.userAnime.user.username.charAt(0).toUpperCase()}</span>}
                            </div>
                        </div>
                        ...
                        <div className="md:flex-col w-full">
                            <div className="flex md:flex-row flex-col md:divide-x md:divide-gray-400">
                                <h1 className="md:pr-2 font-bold text-blue-500 hover:underline">
                                    <Link href={`/user/${r.userId}`}>{r.userAnime.user.username}</Link>
                                </h1>

                                <p className="md:px-2">{r.userAnime.score} {r.userAnime.isFav && (
...
export const DisplayReviews: FC<{ fixedReviews: Review[], user: User | null }> = ({ fixedReviews, user }) => {
    ...
    return fixedReviews.length > 0 && (
        <div className="p-5 bg-base-300 shadow-xl rounded-xl">
            ...
            <div className="grid grid-cols-1 gap-4 mt-5">
                {fixedReviews.map((r: Review) => (
                    <ReviewWrapper key={r.id} r={r} my={user ? r.userAnime.user.username === user.username : false} />
                ))}
            </div>
        </div>
    )
